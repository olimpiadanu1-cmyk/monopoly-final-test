...
function loadGameState() {
    ...
    const gameStateKey = `monopoly_game_state_${currentUser.id}`;
    ...
    if (savedGameState) {
        ...
        console.log('ℹ️ Новая игра - состояние не найдено');
    }
...
// Функция showCartTab удалена - больше не используется
...
function loadCartItems() {
    ...
    if (userRequests.length === 0) {
        container.innerHTML = '<div class="no-data">У вас пока нет запросов</div>';
        return;
    }
    ...
    container.innerHTML = userRequests.map(item => {
        let statusText = '';
        let statusClass = '';
        let dateText = `Заявка от: ${new Date(item.createdAt).toLocaleDateString('ru-RU')}`;
        
        if (item.status === PURCHASE_STATUS.PENDING) {
            statusText = 'ОЖИДАЕТ';
            statusClass = 'status-pending';
        } else if (item.status === PURCHASE_STATUS.APPROVED) {
            statusText = 'ОДОБРЕНО';
            statusClass = 'status-approved';
            dateText = item.approvedAt ? `Одобрено: ${new Date(item.approvedAt).toLocaleDateString('ru-RU')}` : 'Одобрено';
        } else {
...
// Функция loadCartItemsByStatus удалена - больше не используется

// Функция debugAPI удалена
...
// Функции для сохранения данных в JSON файлы
function saveToJsonFile(filename, data) {
    // Поскольку мы не можем напрямую писать файлы из браузера,
    // данные сохраняются только в localStorage
    // JSON файлы обновляются через экспорт/импорт
...
function testDataSaving() {
    ...
    console.log('📝 Заявок на регистрацию:', testData.applications.length);
    console.log('🛒 Заявок на покупку:', testData.purchaseRequests.length);
    console.log('🛍️ Товаров в магазине:', testData.shopItems.length);
    
    return testData;
}

// Экспорт всех данных игры в JSON
function exportAllData() {
    console.log('📤 Экспорт всех данных игры...');
    ...
    const gameData = {
        ...
        // Собираем все состояния игр
        gameStates: {},
        
        // Собираем все корзины
        shoppingCarts: {}
    };
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith('monopoly_game_state_')) {
            const userId = key.replace('monopoly_game_state_', '');
            gameData.gameStates[userId] = JSON.parse(localStorage.getItem(key) || '{}');
        }
    }
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith('monopoly_shopping_cart_')) {
            const userId = key.replace('monopoly_shopping_cart_', '');
            gameData.shoppingCarts[userId] = JSON.parse(localStorage.getItem(key) || '[]');
        }
    }
    ...
    link.download = `monopoly_game_data_${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
...
// Обработка выбранного файла для импорта
function handleImportFile(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (!gameData.users || !Array.isArray(gameData.users)) {
            ...
            localStorage.setItem('monopoly_users', JSON.stringify(gameData.users || []));
            localStorage.setItem('monopoly_applications', JSON.stringify(gameData.applications || []));
            localStorage.setItem('monopoly_purchase_requests', JSON.stringify(gameData.purchaseRequests || []));
            localStorage.setItem('monopoly_cell_tasks', JSON.stringify(gameData.cellTasks || {}));
            localStorage.setItem('monopoly_task_submissions', JSON.stringify(gameData.taskSubmissions || []));
            localStorage.setItem('monopoly_reward_history', JSON.stringify(gameData.rewardHistory || []));
            localStorage.setItem('monopoly_leaderboard', JSON.stringify(gameData.leaderboard || []));
            localStorage.setItem('monopoly_shop_items', JSON.stringify(gameData.shopItems || []));
            
            // Импортируем состояния игр
            if (gameData.gameStates) {
                Object.entries(gameData.gameStates).forEach(([userId, state]) => {
                    localStorage.setItem(`monopoly_game_state_${userId}`, JSON.stringify(state));
                });
            }
            
            // Импортируем корзины
            if (gameData.shoppingCarts) {
                Object.entries(gameData.shoppingCarts).forEach(([userId, cart]) => {
                    localStorage.setItem(`monopoly_shopping_cart_${userId}`, JSON.stringify(cart));
                });
            }
            ...
            applications = gameData.applications || [];
            purchaseRequests = gameData.purchaseRequests || [];
            cellTasks = gameData.cellTasks || {};
